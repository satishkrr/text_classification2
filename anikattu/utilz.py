...
class Averager(list):
    def __init__(self, config, filename=None, ylim=None, clear_list=False):
        super().__init__()
        self.config = config
        self.filename = filename
        self.ylim = ylim
        if filename and not clear_list:
            try:
    ...
    def __str__(self):
        if len(self) > 0:
            ...
            return '\n'.join(['{:0.8f}'.format(i) for i in self])
    ...
    def write_to_file(self):
        ...
        if len(self) > 0 and self.filename:
