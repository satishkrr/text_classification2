...
class Model(Base):
    def __init__(self, config, name,
        ...
        self.train_feed = DataFeed(dataset.name + '.train', self.dataset.trainset,
                                   batchop=self.batchop,
                                   batch_size=self.config.CONFIG.batch_size)
        
        self.test_feed = DataFeed(dataset.name + '.test', self.dataset.testset,
                                  batchop=self.batchop,
                                  batch_size=self.config.CONFIG.batch_size)
...
class AttnModel(Model):
    ...
    def forward(self, input_, return_attn=False):
        ids, (seq,), _ = input_
        if seq.dim() == 1: seq = seq.unsqueeze(0)
        ...
        if return_attn:
            return F.log_softmax(logits, dim=-1), attn
    ...
    def do_predict(self, input_=None):
        ...
        output, attn = self.forward(input_, return_attn=True)
        output = output.max(1)[1].long()
        print(output.size())

        ids, (sequence, ), (label) = input_
        input_string = [self.dataset.input_vocab[i.data[0]] for i in sequence[0]]

        refined_attn_score = [0, ]
        index = 0
        for i, token in enumerate(input_string):
            if token.endswith('@@'):
                refined_attn_score[index] += attn[i]
            else:
                index += 1
                refined_attn_score.append(0)
                refined_attn_score[index] += attn[i]

        input_string = ' '.join(input_string).replace('@@ ', '')
        ...
        refined_attn_score = torch.stack(refined_attn_score).tolist()
        pairs = [(i, j[0][0]) for i,j in zip(input_string.split(), refined_attn_score)]
